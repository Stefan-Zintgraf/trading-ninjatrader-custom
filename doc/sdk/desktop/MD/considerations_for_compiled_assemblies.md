**Path**: [Commercial Distribution](./commercial_distribution.md) > [Considerations for Compiled Assemblies](./considerations_for_compiled_assemblies.md)

# Considerations for Compiled Assemblies

# Considerations for Compiled Assemblies

## [Using Compiled Assemblies](./considerations_for_compiled_assemblies.md)
...
## [Using Custom enum Properties](./considerations_for_compiled_assemblies.md)

When creating custom enum properties, it is advised to create the enum outside of your **NinjaScript** class, and designating it in its own fully qualified namespace. For an example, please see [here](./creating_a_user_defined_parameter_type_enum.md). When using the enum in code, please use the fully qualified namespace as opposed to using a using directive to shorthand the expression.

## [Casting Types in a DLL (Using dynamic Types)](./considerations_for_compiled_assemblies.md)
...
You can set a dynamic object property using the following example:

```csharp
foreach (IDrawingTool region in DrawObjects.ToList())
{
   // Use ToString().Equals() to detect the object's Type
   if (region.ToString().Equals("NinjaTrader.NinjaScript.DrawingTools.RegionHighlightY"))
   {
       //object needs to be cast as an object surrounding it with ( ) to set values
	   ((region) as dynamic).AreaBrush = Brushes.Blue;
   }
}
...
## [Working with the dynamic type](./considerations_for_compiled_assemblies.md)
...
## [Adding XAML and Other Files Into a DLL](./considerations_for_compiled_assemblies.md)
...
## [Exporting custom drawing tools as assembly / DLL](./considerations_for_compiled_assemblies.md)
...
## [Exports might not be backwards compatible](./considerations_for_compiled_assemblies.md)
