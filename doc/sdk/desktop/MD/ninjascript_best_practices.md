**Path**: GUIDES > [NinjaScript Best Practices](./ninjascript_best_practices.md)
...
## [State management practices](./ninjascript_best_practices.md)
...
*   All additional data series must be added in **State.Configure** (this includes series that any hosted script potentially needs as well \- [more info](./adddataseries.md)). Since objects such as **Instrument**, **BarsPeriod**, **TradingHours**, etc. are NOT guaranteed to be available until **State.DataLoaded**, you cannot reliably use the primary instrument properties as arguments in **AddDataSeries()**. Attempting to add a data series dynamically is NOT guaranteed and therefore should be avoided. In some cases, you may be able to use a **BarsRequest()** to obtain market data for other instruments and intervals.
...
## [Error handling practices](./ninjascript_best_practices.md)
...
*   Improper thread handling from a **NinjaScript** object is a common cause of application deadlocks. Please be sure to read more information on [Multi\-Threading Consideration for NinjaScript](./multi_threading_consideration_for_ninjascript.md).
...
## [Performance practices](./ninjascript_best_practices.md)
...
Tip: This is most commonly applicable when using **SharpDX** resources for custom rendering. Please be sure to review the information on [Best Practices for SharpDX Resources](./using_sharpdx_for_custom_chart_rendering.md).
...
## [Miscellaneous practices](./ninjascript_best_practices.md)
...
When creating enums for your **NinjaScript** objects, it is strongly suggested to define those outside the class and in a custom namespace. A reference sample providing all details could be [found here](./creating_a_user_defined_parameter_type_enum.md).
