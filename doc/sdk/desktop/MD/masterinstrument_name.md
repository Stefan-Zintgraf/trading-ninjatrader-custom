**Path**: [MasterInstrument](./masterinstrument.md) > [Name](./masterinstrument_name.md)

# Name

# Name

## [Definition](./masterinstrument_name.md)

Indicates the NinjaTrader database name of an instrument. For example, "MSFT", "ES", "NQ" etc...

## [Property Value](./masterinstrument_name.md)

A **string** representing the name of the instrument.

## [Syntax](./masterinstrument_name.md)

Instrument.MasterInstrument.Name

## [Examples](./masterinstrument_name.md)

```csharp
protected override void OnBarUpdate()

{
  // Displays the master instrument's name at the bottom right of the chart

  Draw.TextFixed(this, "tag1", Bars.Instrument.MasterInstrument.Name, TextPosition.BottomRight);

}

```

![copy](/_next/image?url=%2F_next%2Fstatic%2Fmedia%2FCopy.82606927.svg&w=48&q=75)

## [Additional Access Information](./masterinstrument_name.md)

This property can be accessed without a null reference check in the OnBarUpdate() event handler. When the OnBarUpdate() event is triggered, there will always be an Instrument object. Should you wish to access this property elsewhere, check for null reference first. e.g. if (Instrument != null)

#### ON THIS PAGE

*   [Definition](./masterinstrument_name.md)
*   [Property Value](./masterinstrument_name.md)
*   [Syntax](./masterinstrument_name.md)
*   [Examples](./masterinstrument_name.md)
*   [Additional Access Information](./masterinstrument_name.md)