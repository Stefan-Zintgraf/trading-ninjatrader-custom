**Path**: [Intermediate - RSI with Stop Loss & Profit Target](./intermediate_rsi_with_stop_loss_profit_target.md) > [Working with Brushes](./working_with_brushes.md)
...
## [Understanding predefined brushes](./working_with_brushes.md)
...
![Brushes](https://cdn.sanity.io/images/1hlwceal/production/7ad211b47b7ccc510caa537120398f86b4e39e0c-588x306.png)
...
## [Understanding custom brushes](./working_with_brushes.md)
...
## [Using brushes defined on the user interface](./working_with_brushes.md)
...
Tip: For a complete example of User Definable Color Inputs, please see the reference sample [here](./user_definable_color_inputs.md).
...
![custom_brush](https://cdn.sanity.io/images/1hlwceal/production/556498fbd781983420c9a979804009cea255789a-661x447.png)

## [Using advanced brush types (SharpDX)](./working_with_brushes.md)

### Understanding SharpDX Brushes

While the majority of the NinjaTrader platform's UI is WPF, under the hood, chart's use a DirectX API for faster performance.  To render custom objects to a chart during [OnRender()](./onrender.md), a particular SharpDX Brush object must be implemented which reside in the SharpDX.Direct2D1 namespace.   These brushes can then be passed as arguments to the SharpDX [RenderTarget](./rendertarget.md) methods such [FillRectangle()](./sharpdx_direct2d1_rendertarget_fillrectangle.md), [DrawLine()](./sharpdx_direct2d1_rendertarget_drawline.md), etc.  While SharpDX Brushes behave much the same as previously discussed WPF Brushes, there are a few special considerations you must take as detailed in the following sections.

![note image](/_next/image?url=%2F_next%2Fstatic%2Fmedia%2FNote.57c6078c.svg&w=64&q=75)

## Note

The SharpDX Brushes used in [RenderTarget](./rendertarget.md) methods should NOT be confused with the WPF Brushes used with [DrawingTool Draw](./drawing.md) methods.

### Creating a SharpDX Brush

A [SharpDX Brush](./sharpdx_direct2d1_brush.md) must be created either in OnRender() or RenderTargetChanged().  If you have custom brushes which may be changed on various conditions such as in OnBarUpdate() or by a user during OnStateChange(), or you are pre\-computing a custom brush for performance optimization, you will need to ensure the actual SharpDX instance is updated in OnRender() or RenderTargetChange().
...
Please see [OnRenderTargetChanged()](./onrendertargetchanged.md) for examples of a brush that needs to be recalculated, or [OnRender()](./onrender.md) for an example of recreating a static brush.
...
For convenience, you can convert a computed WPF Brush to a [SharpDX Brush](./sharpdx_direct2d1_brush.md) using the [ToDxBrush(](./todxbrush.md)) extension method.
...
Since SharpDX Brushes reference unmanaged resources, these brushes should always be [disposed](./sharpdx_disposebase.md) of after they have been used.

![note image](/_next/image?url=%2F_next%2Fstatic%2Fmedia%2FWarning.3bcf24ba.svg&w=64&q=75)

## Warning

Failing to dispose of a [SharpDX Brush](./sharpdx_direct2d1_brush.md) and other unmanaged resources can cause the platform to utilize more memory than necessary.
...
These complex types also have an equivalent found in the SharpDX SDK Reference: [SharpDX.Direct2D1.LinearGradientBrush](./sharpdx_direct2d1_lineargradientbrush.md), [SharpDX.Direct2D1.RadialGradientBrush](./sharpdx_direct2d1_radialgradientbrush.md)
